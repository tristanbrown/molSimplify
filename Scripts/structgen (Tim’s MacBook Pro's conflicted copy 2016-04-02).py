...
def getbackbcombs():
    ...
    bbcombs['thd'] = [[1,2,3,4],[3,2,4],[2,4,1],[4,1,3],[3,4],[1,4],[3,2],[1,3],[2,4],[1,2],[1],[2],[3],[4]]
        ...
        for j in range(0,occs[i]):
            ...
            if not(ligand=='x' or ligand =='X') and (totlig-1+denticity < coord):
                ...
                if (denticity == 1):
                    ...
                    urot = uc
                    rdl = vecdiff(auxm.centermass(),lig3D.getAtom(atom0).coords())
                    rdc = vecdiff(m3D.getAtom(batoms[0]).coords(),lig3D.getAtom(atom0).coords())
                    theta,urot = rotation_params(m3D.getAtom(batoms[0]).coords(),lig3D.getAtom(atom0).coords(),auxm.centermass())
                    lig3Db = mol3D()
                    lig3Db.copymol3D(lig3D)
                    lig3D = rotate_around_axis(lig3D,lig3D.getAtom(atom0).coords(),urot,theta)
                    lig3Db = rotate_around_axis(lig3Db,lig3D.getAtom(atom0).coords(),urot,-theta)
...
def structgen(installdir,args,rootdir,ligands,ligoc,globs):
    ...
    if args.bind:
        ...
        conats,planar = core3D.getBondedAtomsnotH(0), False
        if len(conats) > 3:
            for comb in itertools.combinations(conats,4):
                r = [core3D.getAtom(cat).coords() for cat in comb]
                if checkplanar(r[0],r[1],r[2],r[3]):
                    theta0,uplane = rotation_params(r[0],r[1],r[2])
                    planar = True
                    break
        for i in range(0,Nogeom+1):
            ...
            while True:
                ...
                if args.place and 'eq' in args.place:
                    if planar:
                        tr3D = protateref(an3D, Rp, refbP, [R,theta,phi])
                elif args.place and 'ax' in args.place:
                    if planar:
                        tr3D = setPdistanceu(an3D, refbP, uP, R, uplane)
                else:
                    tr3D = protateref(an3D, Rp, refbP, [R,theta,phi])
                ##########################################
                # randomly rotate center of mass
